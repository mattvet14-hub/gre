...
COUNTER_FILE="/etc/gre-tunnels/counter"
...
echo "   GRE Tunnel v3 Setup Script"
echo "===================================="
echo -e "${RESET}"
...
if [[ -f "$SERVICE_PATH" ]]; then
  ...
  # هم اسم رو از رجیستری پاک کن
  sed -i "/^${TUN_LABEL}:/d" "$REGISTRY_FILE" 2>/dev/null || true
  exit 1
fi
...
# خواندن counter و افزایش اتوماتیک
...
# subnet بر اساس counter - کاملاً یونیک و بدون collision
# counter=1 → 10.201.1.0/30
# counter=2 → 10.201.2.0/30
# counter=3 → 10.201.3.0/30 و الی آخر
TUN_IP_FOREIGN="10.201.${COUNTER}.1"
TUN_IP_IRAN="10.201.${COUNTER}.2"

# ذخیره در رجیستری برای مرور بعدی
echo "${TUN_LABEL}: subnet=10.201.${COUNTER}.0/30 | iran=${TUN_IP_IRAN} | foreign=${TUN_IP_FOREIGN}" >> "$REGISTRY_FILE"

echo ""
echo "[i] Tunnel internal IPs:"
echo "    IRAN side   : $TUN_IP_IRAN"
echo "    FOREIGN side: $TUN_IP_FOREIGN"
echo "    Subnet      : 10.201.${COUNTER}.0/30"
echo ""
...
echo "===================================="
echo ""
echo "All tunnels registry: cat $REGISTRY_FILE"
echo ""
